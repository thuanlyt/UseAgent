...
class UsageTelemetryTests(unittest.TestCase):
    ...
    def event(self, event_id: str, agent: str, start: str, end: str, **extra: object) -> dict[str, object]:
        value: dict[str, object] = {
            ...
            "usage": useagent.unavailable_usage("missing", "adapter"),
        }
        value.update(extra)
        return value
    ...
    def test_measured_wall_duration_is_recorded(self) -> None:
        ...
        with useagent.state_lock():
            useagent.record_task_telemetry_locked(
                useagent.load_config(), item, agent, "completed", completed_at="2026-09-07T00:00:01.250Z"
            )
        stored = useagent.load_telemetry(useagent.load_config())["events"]
    ...
    def test_parallel_workers_wall_time_is_not_sum_of_runtime(self) -> None:
        ...
        summary = useagent.aggregate_telemetry(events)
    ...
    def test_authoritative_adapter_usage_is_accepted(self) -> None:
        usage = useagent.normalize_usage_envelope(
            {
                "useagent_usage": 1,
                "authoritative": True,
                "provider": "openai",
    ...
    def test_plain_prose_and_ambiguous_total_are_not_authoritative(self) -> None:
        self.assertEqual(useagent.usage_from_runtime_streams("I used 100 tokens")["provenance"], "unavailable")
        invalid = useagent.normalize_usage_envelope(
            {"useagent_usage": 1, "authoritative": True, "input_tokens": 100, "output_tokens": 40, "total_tokens": 999}
        )
        self.assertEqual(invalid["provenance"], "unavailable")

    def test_missing_usage_is_unavailable_not_zero(self) -> None:
        usage = useagent.normalize_usage_envelope(None)
    ...
    def test_duplicate_event_identity_is_idempotent(self) -> None:
        ...
        with useagent.state_lock():
            useagent.upsert_telemetry_event_locked(config, "task:UA-0105:attempt:1", fields)
            useagent.upsert_telemetry_event_locked(config, "task:UA-0105:attempt:1", fields)
        summary = useagent.load_telemetry_summary(config)
        self.assertEqual(summary["tokens"], 55)
        self.assertEqual(len(useagent.load_telemetry(config)["events"]), 1)
    ...
    def test_retry_keeps_attempts_and_aggregates_usage(self) -> None:
        ...
        summary = useagent.aggregate_telemetry([first, second])
    ...
    def test_takeover_lineage_is_preserved(self) -> None:
        ...
        summary = useagent.aggregate_telemetry([event])
    ...
    def test_participants_are_derived_from_all_event_history(self) -> None:
        ...
        participants = useagent.aggregate_telemetry(events)["participants"]
    ...
    def test_telemetry_store_does_not_persist_raw_prompt_or_secret(self) -> None:
        ...
        usage = useagent.normalize_usage_envelope(payload)
        with useagent.state_lock():
            useagent.upsert_telemetry_event_locked(
                config,
                "cycle:privacy",
                {
                    "kind": "cycle",
        ...
        content = useagent.telemetry_store_path(config).read_text(encoding="utf-8")
    ...
    def test_telemetry_write_does_not_change_source_fingerprint(self) -> None:
        ...
        with useagent.state_lock():
            useagent.upsert_telemetry_event_locked(config, "cycle:fingerprint", {"kind": "cycle", "entity_id": "fingerprint"})
    ...
    def test_runtime_failure_classification_remains_provider_neutral(self) -> None:
        result = useagent.classify_runner_failure(1, "provider says quota", "")
    ...
    def test_report_usage_section_is_concise_and_deterministic(self) -> None:
        summary = useagent.aggregate_telemetry(
            [self.event("UA-0109", "worker-b", "2026-09-07T00:00:00Z", "2026-09-07T00:00:01Z")]
        )
        first = useagent.render_usage_section(summary)
        second = useagent.render_usage_section(summary)
        ...
        with useagent.state_lock():
            useagent.upsert_telemetry_event_locked(
                ...
                "cycle:report",
                {
                    ...
                    "duration_ms": 1000,
                    "usage": useagent.unavailable_usage("missing", "supervisor-runtime"),
                },
            )
        data = useagent.load_registry()
        state = useagent.load_supervisor_state(config)
        report, _ = useagent.build_supervisor_report(config, data, state, "report", [], [], {"status": "not_run"})
        ...
        with contextlib.redirect_stdout(stdout):
            code = useagent.main(
                [
                    ...
                    "--usage-json", json.dumps({"useagent_usage": 1, "authoritative": True, "total_tokens": 9}),
                ]
            )
