...
2. Apply the [Supervisor Judgment and Owner Communication contract](../../../knowledge/contracts/supervisor-judgment.md), then define this cycle's single safe outcome. Run `python tools/useagent.py supervisor cycle` to ingest reports, inspect status, dispatch only ready work whose dependencies are complete and whose writer scope is free, and emit a new report/checkpoint.
