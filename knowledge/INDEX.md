...
2. `$relwit` — supervisor front door; dùng các skill chuyên trách khi cần.
...
- Roster/mailbox: `relwit.config.json` và `work/agents/`.
...
- Dùng `relwit context --task <id>` để lấy snapshot ngắn.
...
- `next_refresh`: sau khi thêm module, public contract, build command hoặc deploy path; telemetry contract hiện được ghi tại `knowledge/contracts/usage-telemetry.md` và `knowledge/decisions/0010-usage-telemetry-and-execution-summary.md`.
